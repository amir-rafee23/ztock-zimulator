...
let arrow_clicked st dir =
  ...
  in
  match st with
  | { screen; selected = _; portfolio = p; ticker; quantity } ->
      { screen; selected = sel; portfolio = p; ticker; quantity }
...
let enter_clicked st =
  ...
  | to_scr -> (
      match st with
      | { screen = _; selected = _; portfolio; ticker; quantity } ->
          { screen = to_scr; selected = 0; portfolio; ticker; quantity })

let character_clicked st c =
  match st with
  | { screen = Buy Ticker; selected; portfolio; ticker; quantity } ->
      {
        screen = Buy Ticker;
        selected;
        portfolio;
  ...
  | { screen = Buy Quantity; selected; portfolio; ticker; quantity } ->
      {
        screen = Buy Quantity;
        selected;
        portfolio;
